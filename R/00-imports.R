#' @importFrom rlang is_character list2 !! !!! is_missing inherits_any inherits_all abort warn is_null
#' @importFrom rlang set_names enquo quo_get_env quo_get_expr sym
#' @importFrom zeallot %->%
#' @importFrom assertthat assert_that
#' @importFrom vctrs vec_in vec_c vec_size vec_rbind vec_assert %0% vec_recycle vec_is as_list_of
#' @importFrom dplyr %>% bind_rows
#' @importFrom magrittr multiply_by raise_to_power subtract equals not %<>%
#' @importFrom tibble tibble
#' @importFrom purrr map map_dbl map2 imap pmap walk2
#' @importFrom forcats as_factor fct_cross
#' @importFrom grid gpar
NULL


# To be deprecated
#' @importFrom foreach foreach %do%
#' @importFrom grDevices contourLines
#' @importFrom MASS kde2d
#' @importFrom grid is.unit unit
#' @import ggplot2

NULL